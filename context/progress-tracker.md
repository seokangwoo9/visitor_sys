...
- Admin dashboard UI redesigned with a refined enterprise visual language: tighter card radii (`rounded-xl`/`rounded-lg`), smaller and calmer typography hierarchy, reduced eyebrow tracking, lighter sidebar (`w-60 bg-bg-base`), softer nav active state (`bg-visitor-success-soft`), denser metric cards, cleaner bar charts, and a plain page-title `AdminHero` replacing the boxed card header. All green brand tokens retained; no logic or data changes.
- Export section "Report Includes" info panel removed; export card is now full-width.
- Settings QR card fixed (was cramped): the two visitor QR codes now stack vertically instead of squeezing side-by-side, with refined-enterprise styling applied and the Settings grid balanced to equal columns.
- Visitor-facing pages (registration/check-in, check-out, visitor status) redesigned to match the refined enterprise language: clean white brand header replacing the dark ink block, tighter radii, calmer typography, smaller icon badges, and green (not dark) primary buttons on check-out.
- Converted deployment to Docker-first: added multi-stage `Dockerfile`, `docker-entrypoint.sh` (runs `prisma migrate deploy` on start), `.dockerignore`, base `docker-compose.prod.yml` (app + postgres), `docker-compose.prod.https.yml` (Caddy auto-HTTPS overlay), `deploy/Caddyfile`, `deploy/prod.env.example`, and `DEPLOY_VPS_UBUNTU_DOMAIN_HTTPS.md`. The app, PostgreSQL, and Caddy reverse proxy now all run as containers; Caddy is the HTTPS entry point. Local `compose.yaml` still runs Postgres only for development.
- Added `DEPLOYMENT_GUIDE.md`: a detailed, beginner-oriented start-to-finish VPS deployment walkthrough (fresh Ubuntu server → non-root user → firewall → Docker → clone → env secrets → build/start → admin seed → verify → QR printing → operations, updates, backups, troubleshooting) built on the Docker-first Caddy stack.
- Adopted host + VM deployment (Option A) as the primary topology: the app runs HTTP-only in a VM and an upstream host reverse proxy (sharing one public IP) terminates HTTPS and routes by hostname. Made the app port bind configurable via `APP_BIND`/`APP_PORT` in `docker-compose.prod.yml` so it can be pinned to the VM's private interface; reframed `DOMAIN`/`ACME_EMAIL` in `deploy/prod.env.example` as Option-B-only (in-VM Caddy). Rewrote `DEPLOYMENT_GUIDE.md` for the host+VM flow, including host nginx/Caddy reverse-proxy examples and the required `X-Forwarded-Proto: https` forwarding. No application code changed (app has no server actions and derives its public URL from `BETTER_AUTH_URL`). The in-VM Caddy overlay remains available for standalone VMs (Option B).
- Admin dashboard UI redesigned with a refined enterprise visual language: tighter card radii (`rounded-xl`/`rounded-lg`), smaller and calmer typography hierarchy, reduced eyebrow tracking, lighter sidebar (`w-60 bg-bg-base`), softer nav active state (`bg-visitor-success-soft`), denser metric cards, cleaner bar charts, and a plain page-title `AdminHero` replacing the boxed card header. All green brand tokens retained; no logic or data changes.
- Client-side i18n language switching added to visitor registration page with English, Malay (Bahasa Melayu), and Chinese Simplified support. Language selector appears at the top of the registration form. User selection persists to localStorage. Translation applied to all form labels, placeholders, section titles, buttons, and error messages.
- Visitor-facing Safety Acknowledgment title now opens the full acknowledgment modal directly from the checkbox row, replacing the separate title box.
- PDPA consent capture added to check-in as a required acceptance, mirroring the Safety Acknowledgment version-based policy pattern. New `PdpaConsentVersion` model (with migration), `repositories/pdpa-consent-repository.ts`, `services/pdpa-consent-service.ts`, `lib/pdpa-consent.ts` defaults, public `GET /api/visitor/pdpa-consent`, and admin `POST /api/admin/pdpa-consent`. The registration form shows a required PDPA checkbox with a modal of the active policy text; submission is server-side version-validated (409 on version change). Admins edit the PDPA text from Settings via `AdminPdpaConsentForm`. Visitor detail dialog and Excel export now include PDPA consent, consented-at, and version columns.
...
- Fixed critical timezone issue: migrated all `TIMESTAMP(3)` columns to `TIMESTAMPTZ` (timestamp with time zone) to prevent 8-hour timezone shifts. The previous schema stored timestamps without timezone info, causing JavaScript to misinterpret UTC timestamps as local time (GMT+8). After migration, all timestamps are stored with explicit UTC timezone (`+00`).
- Fixed the root cause of production times displaying as UTC instead of Malaysia time: the Docker container ran in the default UTC timezone even though the host server was `Asia/Kuala_Lumpur`. Set `ENV TZ=Asia/Kuala_Lumpur` in the Dockerfile runner stage and added the `tzdata` package so server-side `Date` math (daily/weekly/monthly metric buckets, `startOfDay`/`startOfWeek` in `services/admin-visitor-service.ts`) operates in local time. Also pinned `timeZone: "Asia/Kuala_Lumpur"` and `hour12: false` in every `Intl.DateTimeFormat` call that renders a timestamp (`app/admin/page.tsx`, `app/api/admin/export/route.ts`, `app/visitor/status/page.tsx`, `app/check-out/active-visit-check-out-form.tsx`) so display formatting is correct regardless of container/host timezone. Requires a `--no-cache` Docker image rebuild on production for the timezone env var to take effect.
