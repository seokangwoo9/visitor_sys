...
import type { PdpaConsentPolicy } from "@/types/visitor";

interface AdminPdpaConsentFormProps {
  pdpaConsent: PdpaConsentPolicy;
}

export function AdminPdpaConsentForm({ pdpaConsent }: AdminPdpaConsentFormProps) {
  ...
  async function handleSubmit(formData: FormData) {
    ...
    if (!response.ok) {
      setMessage("Unable to publish PDPA consent.");
      return;
    }

    setMessage("PDPA consent published.");
    startTransition(() => {
      router.refresh();
    });
  }
      ...
      <div className="grid gap-4">
        <label className="space-y-2">
          ...
          <Input
            ...
            defaultValue={pdpaConsent.title}
            maxLength={200}
            name="title"
            required
          />
        ...
        <label className="space-y-2">
          ...
          <Textarea
            ...
            defaultValue={pdpaConsent.content}
            maxLength={10000}
            name="content"
            required
          />
